-- Remplace uniquement le visuel bleu par la version fidèle au marquage
-- « marque L. VUITTON déposée ». Idempotent et sans modification de stock.
...
set data=replace(
  data::text,
  'assets/products/lv-damier-bleu-studio.webp',
  'assets/products/lv-damier-bleu-studio-v2.webp'
)::jsonb,
updated_at=now()
where id='lv-damier-bleu'
  and data::text like '%assets/products/lv-damier-bleu-studio.webp%';

with latest as (
  select id,data from public.admin_drafts order by updated_at desc limit 1 for update
), refreshed as (
  select d.id,jsonb_agg(
    case when e.product->>'id'='lv-damier-bleu' then p.data else e.product end
...
select id,data->>'img' image,
       data->'valueImages'->>'Coloris::Bleu' coloris_image
from public.products where id='lv-damier-bleu';
